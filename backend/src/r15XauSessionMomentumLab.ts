...
interface Param { family:Family; session:SessionName; windowMin:number; trigger:number; stopAtr:number; rr:number; holdMin:number; }
...
async function main(){
  const endDay=Math.floor((Date.now()-DAY)/DAY)*DAY,end=endDay+DAY-1,start=end-LOOKBACK_DAYS*DAY;
  ...
  const first=bars[0].time,last=bars.at(-1)!.time,span=last-first,s1=first+span*.5,s2=first+span*.75;
  const i1=bars.findIndex(b=>b.time>=s1),i2=bars.findIndex(b=>b.time>=s2),endIdx=bars.length-2;
  const grid=buildGrid(),trained:Candidate[]=[];
  for(const p of grid){const train=simulate(bars,220,i1-1,p);if(train.trades<10||train.returnPct<=0||train.pf<1.03||train.dd>15)continue;const validation=simulate(bars,i1,i2-1,p),score=train.returnPct-train.dd*.8+Math.min(train.pf,3)*3+Math.min(train.trades,100)*.01;trained.push({p,train,validation,score});}
...
function buildGrid():Param[]{const out:Param[]=[];
  ...
  for(const session of ['LONDON','NEW_YORK','OVERLAP'] as const)for(const windowMin of [30,60,120])for(const trigger of [6,12,24])for(const stopAtr of [1,1.5,2])for(const rr of [1.5,2,3])out.push({family:'SESSION_DONCHIAN',session,windowMin,trigger,stopAtr,rr,holdMin:120});return out;}

function simulate(f:Feature[],start:number,end:number,p:Param):Metrics{let balance=INITIAL,peak=INITIAL,dd=0,wins=0,gp=0,gl=0,trades=0,i=Math.max(220,start);while(i<end-2&&balance>1){const sig=signal(f,i,p);if(!sig){i++;continue;}const entryIdx=i+1;if(entryIdx>end)break;const b=f[entryIdx],entry=b.open*(1+sig.side*(SPREAD_PCT/2+SLIP_PCT)/100),stopDist=Math.max(entry*.0006,f[i].atr*p.stopAtr),tpDist=Math.max(stopDist*p.rr,entry*((FEE_PCT*2+SPREAD_PCT+SLIP_PCT*2)*1.15)/100),sl=entry-sig.side*stopDist,tp=entry+sig.side*tpDist,notional=balance*MARGIN_PCT/100*LEVERAGE,qty=notional/entry,entryFee=notional*FEE_PCT/100;let exitIdx=Math.min(end,entryIdx+Math.max(1,Math.floor(p.holdMin/5))),exit=f[exitIdx].close;for(let j=entryIdx;j<=exitIdx;j++){const x=f[j],hitSl=sig.side===1?x.low<=sl:x.high>=sl,hitTp=sig.side===1?x.high>=tp:x.low<=tp;if(hitSl){exit=sl;exitIdx=j;break;}if(hitTp){exit=tp;exitIdx=j;break;}}exit*=1-sig.side*(SPREAD_PCT/2+SLIP_PCT)/100;const gross=sig.side===1?(exit-entry)*qty:(entry-exit)*qty,exitFee=qty*exit*FEE_PCT/100,pnl=gross-entryFee-exitFee;balance+=pnl;trades++;if(pnl>0){wins++;gp+=pnl}else gl+=Math.abs(pnl);peak=Math.max(peak,balance);dd=Math.max(dd,(peak-balance)/peak*100);i=Math.max(i+1,exitIdx+1);}return{trades,wins,winRate:trades?wins/trades*100:0,pf:gl>0?gp/gl:gp>0?99:0,returnPct:(balance-INITIAL)/INITIAL*100,dd,balance,expectancy:trades?(balance-INITIAL)/trades:0};}

function signal(f:Feature[],i:number,p:Param):{side:Side}|null{const b=f[i],parts=sessionParts(b.time,p.session),start=parts.start,end=parts.end;if(b.time<start||b.time>end)return null;
  if(p.family==='FIRST_WINDOW_MOMENTUM'){const windowEnd=start+p.windowMin*MIN;if(b.time!==windowEnd)return null;const ws=smallWindow(f,i,start,windowEnd);if(ws.length<Math.floor(p.windowMin/5))return null;const ret=(ws.at(-1)!.close-ws[0].open)/ws[0].open*100;if(Math.abs(ret)<p.trigger)return null;const side:Side=ret>0?1:-1,trend=side===1?b.ema20>b.ema50:b.ema20<b.ema50;return trend?{side}:null;}
  if(p.family==='OPENING_RANGE_BREAKOUT'){const windowEnd=start+p.windowMin*MIN;if(b.time<windowEnd||b.time>windowEnd+120*MIN)return null;const ws=smallWindow(f,i,start,windowEnd);if(ws.length<Math.floor(p.windowMin/5))return null;const hi=Math.max(...ws.map(x=>x.high)),lo=Math.min(...ws.map(x=>x.low)),buffer=b.atr*p.trigger,long=b.close>hi+buffer&&b.close>b.open&&b.ema20>b.ema50,short=b.close<lo-buffer&&b.close<b.open&&b.ema20<b.ema50;return long||short?{side:long?1:-1}:null;}
  const lookback=Math.floor(p.trigger),prior=f.slice(Math.max(0,i-lookback),i);if(prior.length<lookback)return null;const hi=Math.max(...prior.map(x=>x.high)),lo=Math.min(...prior.map(x=>x.low)),trendLong=b.ema20>b.ema50&&b.ema50>b.ema200,trendShort=b.ema20<b.ema50&&b.ema50<b.ema200,long=trendLong&&b.close>hi&&b.volume>=b.volAvg*.9,short=trendShort&&b.close<lo&&b.volume>=b.volAvg*.9;return long||short?{side:long?1:-1}:null;}

function smallWindow(f:Feature[],i:number,start:number,end:number){const bars=Math.ceil((end-start)/FIVE)+3;return f.slice(Math.max(0,i-bars),i+1).filter(x=>x.time>=start&&x.time<end);}
function sessionParts(time:number,session:SessionName){const d=new Date(time),dayStart=Date.UTC(d.getUTCFullYear(),d.getUTCMonth(),d.getUTCDate());if(session==='LONDON')return{start:dayStart+7*60*MIN,end:dayStart+11*60*MIN};if(session==='NEW_YORK')return{start:dayStart+13*60*MIN,end:dayStart+18*60*MIN};return{start:dayStart+12*60*MIN,end:dayStart+17*60*MIN};}
function features(rows:Candle[]):Feature[]{const close=rows.map(x=>x.close),e20=ema(close,20),e50=ema(close,50),e200=ema(close,200),a=atr(rows,14),r=rsi(close,14);return rows.map((x,i)=>({...x,atr:a[i],ema20:e20[i],ema50:e50[i],ema200:e200[i],rsi:r[i],volAvg:mean(rows.slice(Math.max(0,i-19),i+1).map(y=>y.volume))}));}
