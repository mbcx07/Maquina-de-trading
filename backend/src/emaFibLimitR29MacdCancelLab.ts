...
type R31Cfg={scoreMin:number;reverseBars:number;macdMin:number;tpKind:'PCT'|'ATR'|'STRUCTURE';tpValue:number;slKind:'PCT'|'ATR'|'STRUCTURE';slValue:number;partial:boolean};
type R31Signal={i:number;side:Side;score:number;engine:string};
function r31Signals(p:Prepared,scoreMin:number){const out:R31Signal[]=[];for(let i=160;i<p.bars.length-1;i++){const b=p.bars,x=b[i],up=p.ema8[i]>p.ema14[i],dn=p.ema8[i]<p.ema14[i],crossUp=up&&p.ema8[i-1]<=p.ema14[i-1],crossDn=dn&&p.ema8[i-1]>=p.ema14[i-1],vol=p.avgVolume[i]?p.filterVolume[i]/p.avgVolume[i]:0;let side:Side|null=null,engine='',base=0;if(crossUp){side='BUY';engine='EMA_CROSS';base=2}else if(crossDn){side='SELL';engine='EMA_CROSS';base=2}else if(up&&x.low<=p.ema8[i]&&x.close>p.ema8[i]&&x.close>x.open){side='BUY';engine='EMA_BOUNCE';base=1.5}else if(dn&&x.high>=p.ema8[i]&&x.close<p.ema8[i]&&x.close<x.open){side='SELL';engine='EMA_BOUNCE';base=1.5}else if(up&&x.low<=p.ema150[i]&&x.close>p.ema150[i]){side='BUY';engine='EMA150_PULLBACK';base=2}else if(dn&&x.high>=p.ema150[i]&&x.close<p.ema150[i]){side='SELL';engine='EMA150_PULLBACK';base=2}else{let hi=-Infinity,lo=Infinity;for(let j=i-20;j<i;j++){hi=Math.max(hi,b[j].high);lo=Math.min(lo,b[j].low)}if(x.high>hi&&x.low<=hi&&x.close>hi){side='BUY';engine='BREAK_RETEST';base=2.5}else if(x.low<lo&&x.high>=lo&&x.close<lo){side='SELL';engine='BREAK_RETEST';base=2.5}}if(!side||p.trend[i]!==side||(side==='BUY'?x.close<=p.ema150[i]:x.close>=p.ema150[i])||vol<1)continue;const score=base+Math.min(2,vol-1)+Math.min(1,Math.abs(p.ema8[i]-p.ema14[i])/Math.max(p.atr[i],1e-9));if(score>=scoreMin)out.push({i,side,score,engine})}return out}
function r31Macd(base:Bar[],p:Prepared,tf:number){if(!tf)return null;const h=reaggregate(base,tf*60000),c=h.map(x=>x.close),a=emaSeries(c,12),z=emaSeries(c,26),m=Float64Array.from(c,(_,i)=>a[i]-z[i]),s=emaSeries(Array.from(m),9);return{m:mapClosedSeries(p.bars,1,h,tf,m),s:mapClosedSeries(p.bars,1,h,tf,s)}}
function r31Sim(p:Prepared,sigs:R31Signal[],cfg:R31Cfg,start:number,end:number,macd:ReturnType<typeof r31Macd>){const b=p.bars;let eq=100,pk=100,dd=0,gp=0,gl=0,n=0,w=0,next=start;for(const q of sigs){const i=q.i;if(i<start||i>=end-1||i<next)continue;const side=q.side,entry=b[i+1].open,a=p.atr[i];let lo=Infinity,hi=-Infinity;for(let j=Math.max(0,i-10);j<=i;j++){lo=Math.min(lo,b[j].low);hi=Math.max(hi,b[j].high)}let lo60=Infinity,hi60=-Infinity;for(let j=Math.max(0,i-60);j<=i;j++){lo60=Math.min(lo60,b[j].low);hi60=Math.max(hi60,b[j].high)}const sl=cfg.slKind==='PCT'?(side==='BUY'?entry*(1-cfg.slValue/100):entry*(1+cfg.slValue/100)):cfg.slKind==='ATR'?(side==='BUY'?entry-a*cfg.slValue:entry+a*cfg.slValue):(side==='BUY'?lo-a*.1:hi+a*.1);let tp=cfg.tpKind==='PCT'?(side==='BUY'?entry*(1+cfg.tpValue/100):entry*(1-cfg.tpValue/100)):cfg.tpKind==='ATR'?(side==='BUY'?entry+a*cfg.tpValue:entry-a*cfg.tpValue):(side==='BUY'?hi60:lo60);if(side==='BUY'&&tp<=entry||side==='SELL'&&tp>=entry)tp=side==='BUY'?entry+a*2:entry-a*2;const tp1=entry+(tp-entry)*.5;let exit=entry,xi=i+1,rev=0,partialDone=false,grossPart=0;for(let j=i+1;j<end;j++){const y=b[j],hs=side==='BUY'?y.low<=sl:y.high>=sl,ht1=cfg.partial&&!partialDone&&(side==='BUY'?y.high>=tp1:y.low<=tp1),ht=side==='BUY'?y.high>=tp:y.low<=tp;if(hs){exit=partialDone?entry*(side==='BUY'?1+COST/100:1-COST/100):sl;xi=j;break}if(ht){exit=tp;xi=j;break}if(ht1){grossPart=(side==='BUY'?(tp1/entry-1):(entry/tp1-1))*50;partialDone=true}const opp=side==='BUY'?p.ema8[j]<p.ema14[j]:p.ema8[j]>p.ema14[j];rev=opp?rev+1:0;const macdOk=!macd||(side==='BUY'?macd.m[j]<macd.s[j]:macd.m[j]>macd.s[j]);if(rev>=cfg.reverseBars&&macdOk){exit=y.close;xi=j;break}if(j===end-1){exit=y.close;xi=j}}let gross=(side==='BUY'?(exit/entry-1):(entry/exit-1))*100;if(partialDone)gross=grossPart+gross*.5;const net=gross-COST,ar=net*EXPOSURE;eq*=1+ar/100;pk=Math.max(pk,eq);dd=Math.max(dd,(pk-eq)/pk*100);n++;if(ar>0){w++;gp+=ar}else gl+=Math.abs(ar);next=xi+1}const days=Math.max(1,(b[Math.min(end-1,b.length-1)].time-b[Math.max(start,0)].time)/DAY);return{trades:n,winRate:n?w/n*100:0,pf:gl?gp/gl:gp?99:0,returnPct:eq-100,ddPct:dd,tradesPerDay:n/days,final50:eq/2} as Stats}
async function mainR31(){const end=Math.floor((Date.now()-2*DAY)/DAY)*DAY,start=end-DAYS*DAY,dates:string[]=[];for(let t=start;t<end;t+=DAY)dates.push(new Date(t).toISOString().slice(0,10));const daily=await mapLimit(dates,DOWNLOAD_CONCURRENCY,fetchDay),base=daily.flat().sort((a,b)=>a.time-b.time),p=prepareR30(base,1,15,.5,1),t1=Math.floor(p.bars.length*.5),t2=Math.floor(p.bars.length*.75),macds=new Map<number,ReturnType<typeof r31Macd>>();for(const x of [0,3,5,15])macds.set(x,r31Macd(base,p,x));const rows:Array<{cfg:R31Cfg;train:Stats;validation:Stats;score:number}>=[];for(const scoreMin of [2,2.5,3]){const sigs=r31Signals(p,scoreMin);for(const reverseBars of [1,2,3])for(const macdMin of [0,3,5,15])for(const tp of [['PCT',.2],['PCT',.3],['PCT',.5],['ATR',1],['ATR',2],['STRUCTURE',0]] as const)for(const sl of [['PCT',.15],['PCT',.3],['ATR',1],['ATR',2],['STRUCTURE',0]] as const)for(const partial of [false,true]){const cfg:R31Cfg={scoreMin,reverseBars,macdMin,tpKind:tp[0],tpValue:tp[1],slKind:sl[0],slValue:sl[1],partial},train=r31Sim(p,sigs,cfg,0,t1,macds.get(macdMin)!),validation=r31Sim(p,sigs,cfg,t1,t2,macds.get(macdMin)!);rows.push({cfg,train,validation,score:scoreR27(train)+scoreR27(validation)*1.5})}}rows.sort((a,b)=>b.score-a.score);const eligible=rows.filter(x=>x.train.tradesPerDay>=10&&x.validation.tradesPerDay>=10),best=eligible[0]??rows[0],sigs=r31Signals(p,best.cfg.scoreMin),test=r31Sim(p,sigs,best.cfg,t2,p.bars.length,macds.get(best.cfg.macdMin)!);await mkdir('artifacts',{recursive:true});await writeFile('artifacts/r31-entry-exit-grid.json',JSON.stringify({tested:rows.length,eligible:eligible.length,best,test,top20:rows.slice(0,20)},null,2));console.log('R31_RESULT',JSON.stringify({testedConfigurations:rows.length,frequencyEligible:eligible.length,config:best.cfg,train:best.train,validation:best.validation,test,requirements:{winRateMin:65,tradesPerDayMin:10,pfMin:1.2},survived:passR27(test)}))}
mainR31().catch((e) => {
  console.error(e);
  process.exitCode = 1;
});
