...
function simulate(f:Feature[],start:number,end:number,p:Params):Metrics{
 ...
 while(i<end-1){const s=f[i];const hour=new Date(s.time).getUTCHours();if(p.session==='LIQUID'&&(hour<6||hour>=21)){i++;continue;}if(s.atrPct<COST_TOTAL()*p.volRatio){i++;continue;}
   ...
   const tp=entry*(1+side*targetPct/100);const sl=entry*(1-side*stopPct/100);const margin=balance*COST.marginPct/100;const notional=margin*COST.leverage;const qty=notional/entry;const entryFee=notional*COST.feePct/100;
   let exitIndex=Math.min(end,i+1+p.hold),exit=f[exitIndex].close;for(let j=i+1;j<=exitIndex;j++){const b=f[j];const hitSl=side===1?b.low<=sl:b.high>=sl;const hitTp=side===1?b.high>=tp:b.low<=tp;if(hitSl){exit=sl;exitIndex=j;break;}if(hitTp){exit=tp;exitIndex=j;break;}}
...
function ema(v:number[],p:number):number[]{const a=2/(p+1),o=new Array(v.length);if(!v.length)return[];o[0]=v[0];for(let i=1;i<v.length;i++)o[i]=v[i]*a+o[i-1]*(1-a);return o}
