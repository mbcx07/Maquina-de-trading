...
def main():
    ...
    output={"mode":"LOSS_ATTRIBUTION_DEVELOPMENT_ONLY","symbols":list(SYMBOLS),"configuration":{"timeframe":FRAME,"tpPct":.30,"sl":"ATR4 capped .70","breakEven":True},"samples":{"train":len(train),"validationA":len(va),"validationB":len(vb)},"baseline":{"train":metrics(train),"validationA":metrics(va),"validationB":metrics(vb)},"attribution":{"bySymbolTrain":groups(train,"symbol"),"byHourTrain":groups(train,"hourBlock"),"byPatternTrain":groups(train,"pattern"),"featureImportanceValidationA":importance},"entryDecision":{"probabilityThreshold":threshold,"train":metrics(chosen[2]),"validationA":metrics(chosen[3]),"validationB":metrics(fb),"validationBRetained":len(fb)/len(vb) if vb else 0},"lookaheadFree":True,"blindOpened":False}
